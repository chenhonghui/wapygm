MYAPP.ready(function(){
    ...
    function initPage(page,curpage){
        ...
        $.ajax({
            type:'post',
            url:ApiUrl+"/index.php?act=member_order&op=order_search&page="+page+"&curpage="+curpage+'&keyword='+keyword,
            data:{key:key},
            dataType:'json',
...
//                merge_payment(order_state);
                ...
                // MYAPP.jump('openQQ', {"QQ": "3149611689"});
                //});
                //提交单个订单
    ...
    //合并付款
    function merge_payment(order_state){
        if(order_state == '10'){
            ...
            $("#allChecked").click(allChecked); //全选
            $("#allSub").on("click",allSub);//提交
        }else{
    ...
    function examineCheckPay(){
        ...
        if(!allBox.length){
            ...
            allBox.each(function(index){
                var t=this;
                if($(this).hasClass("pay_checked")){
                    var pay_amount = $(this).parents('.one_order').find('.pay_amount');
                    var amount = parseFloat($(pay_amount).html());
                    if(!isNaN(amount)){
                        order_count += amount;
                    }
                    order_sum++;
        ...
        $('#order_count').html(order_count.toFixed(2));
        $('#order_sum').html(order_sum);
        return end
    }
    ...
    function allChecked(){
        ...
        //全部处于选择状态则全不选
        if(allBtn.hasClass("pay_checked")&&examineCheckPay()){
                            ...
                            $.ajax({
                                ...
                                success:function(result){
                                    ...
                                    if(friend_info.error){
                                        ...
                                        $('#send_sms').attr('href','sms:'+(isIos()?'&':'?')+'body=我在陌云商城买了'+friend_info.goods_sum+'件商品，需付款¥'+friend_info.pay_amount+'，是不是真感情，就看你戳不戳下面这个付款链接了，快来帮我付款吧～'+friend_info.pay_url).trigger('click');;
                                    }
                                }
                            });
