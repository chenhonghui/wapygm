...
function regionInit(divId) {
    ...
    getArea(function() {
        ...
        $("#" + divId + " > input[type='button'][class='edit_region']").click(regionEdit); // 编辑按钮的onclick事件
    });
}
...
if (typeof(regionChange) != 'function') { //检测是否已经被定义过，防止重写
    function regionChange() {
        for (var j = $(this); j.next("select").length; j.next("select").remove());
...
function regionEdit() {
    ...
    $(this).siblings("span").push(this).hide();
}
...
function gcategoryChange() {
    ...
    // 计算当前选中到id和拼起来的name
    var selects = $(this).siblings("select").push(this);
    var id = 0;
    var names = new Array();
    for (i = 0; i < selects.length; i++) {
        sel = selects[i];
...
function gcategoryEdit() {
    $(this).siblings("select").show();
    $(this).siblings("span").push(this).remove();
}
...
function getArea(callback) {
    if (typeof(nc_a) == 'undefined') { //加载地区数据
        ...
        area_scripts_src = $("script[src*='zepto.min.js']").attr("src"); //取JS目录的地址
        area_scripts_src = area_scripts_src.replace('zepto.min.js', 'area_array.js');
        $.getScript(area_scripts_src,
        function() {
            callback();
            return true;
        });
